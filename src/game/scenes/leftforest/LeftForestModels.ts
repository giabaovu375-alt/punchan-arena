...
function normalizeScale(g: THREE.Group, targetH: number, multiplier = 1.0) {
  ...
  if (h < 0.001) return;
  if (h > 100) h /= 100; // FBX cm → mét
  g.scale.setScalar((targetH / h) * multiplier);
}

function groundSnap(g: THREE.Group) {
  const bbox = new THREE.Box3().setFromObject(g);
  if (isFinite(bbox.min.y)) g.position.y = -bbox.min.y;
}
...
// ── FIX: truyền scene vào cloneAndPlace ──────────────────────────────────────
function cloneAndPlace(
  ...
  shadow(first);
  normalizeScale(first, scaleBase + Math.random() * scaleVar);
  ...
  for (let i = 1; i < positions.length; i++) {
    const c = first.clone();
    normalizeScale(c, scaleBase + Math.random() * scaleVar);
...
export async function loadAllModels(
  ...
  const gltfLoad = (name: string, base = MODEL_BASE) =>
    new Promise<THREE.Group>((res, rej) =>
      loader.load(`${base}/${name}.gltf`, g => res(g.scene), undefined, rej));
  ...
  for (let i = 0; i < 45; i++) {
    const angle = (i / 45) * Math.PI * 2 + Math.random() * 0.18;
  ...
  for (const anchor of clusterAnchors) {
    ...
    for (let j = 0; j < count; j++) {
      ...
      if (!src) continue;
      const t = src.clone();
      shadow(t); nightify(t, 0.38, new THREE.Color(0.55, 0.9, 0.65));
  ...
  for (const anchor of clusterAnchors) {
    ...
    for (let j = 0; j < count; j++) {
      ...
      if (!src) continue;
      const g = src.clone();
      shadow(g); nightify(g, 0.5, new THREE.Color(0.6, 1.0, 0.7));
  ...
  const rockModels = await Promise.all(
    [...rockNames, ...pebNames].map(n => gltfLoad(n).catch(() => null))
  );
  ...
  for (let z = -44; z < 44; z += 6) {
    for (const side of [-1, 1]) {
      ...
      if (!src) continue;
      const r = src.clone();
      shadow(r); nightify(r, 0.32, new THREE.Color(0.7, 0.85, 0.8));
  ...
  for (const pos of bigRockPositions) {
    ...
    if (!src) continue;
    const r = src.clone();
    shadow(r); nightify(r, 0.3, new THREE.Color(0.68, 0.82, 0.78));
    normalizeScale(r, CFG.MODEL_SCALE.rockH * 2.5);
  ...
  // ── Props async — FIX: truyền scene vào cloneAndPlace ─────────────────────
  ...
  propLoad(`${PROP_BASE}/pile_of_skulls.glb`).then(skulls => {
    ...
    cloneAndPlace(scene, skulls, positions, CFG.MODEL_SCALE.skulls, 0,
      [0.45, new THREE.Color(0.85, 0.78, 0.7)]);
  }).catch(e => console.error("❌ skulls:", e));
  ...
  propLoad(`${PROP_BASE}/stylized_fence.glb`).then(fence => {
    ...
    cloneAndPlace(scene, fence, positions1, CFG.MODEL_SCALE.fence, 0,
      [0.42, new THREE.Color(0.7, 0.65, 0.55)]);
    cloneAndPlace(scene, fence, positions2, CFG.MODEL_SCALE.fence, 0,
      [0.42, new THREE.Color(0.7, 0.65, 0.55)]);
  }).catch(e => console.error("❌ fence:", e));
  ...
  }).catch(e => console.error("❌ statue:", e));
  ...
  propLoad(`${PROP_BASE}/Big-stone.glb`).then(stone => {
    ...
    scene.add(stone);
  }).catch(e => console.error("❌ big-stone:", e));
  ...
  propLoad(`${PROP_BASE}/low_poly_lamp_post.glb`).then(lamp => {
    ...
    const lampLeft  = [-28, -10, 8, 26];
    const lampRight = [-20, 0, 20];
    for (const z of lampLeft) {
      ...
      scene.add(pt); disposableLights.push(pt);
    }
  ...
  propLoad(`${PROP_BASE}/stone_pillar.glb`).then(pillar => {
    ...
    cloneAndPlace(scene, pillar, positions, CFG.MODEL_SCALE.bigStone * 1.4, 0,
      [0.36, new THREE.Color(0.72, 0.82, 0.78)]);
  }).catch(e => console.error("❌ pillar:", e));

  // ── Crystal xanh — đặt thành cụm, scale nhỏ hợp lý ──────────────────────
  propLoad(`${PROP_BASE}/crystal_cluster.glb`).then(crystal => {
    const centers = [
      { x: -24, z: -15 }, { x: 20, z: -38 }, { x: -32, z: 40 }, { x: 28, z: 18 },
    ];
    for (const center of centers) {
      // 1 crystal lớn ở giữa
      const main = crystal.clone();
      shadow(main); nightify(main, 0.7, new THREE.Color(0.4, 1.0, 0.6));
      normalizeScale(main, 1.8); // cao ~1.8m
      main.position.set(center.x * WORLD_SCALE, 0, center.z * WORLD_SCALE);
      groundSnap(main);
      main.rotation.y = Math.random() * Math.PI * 2;
      scene.add(main);

      // 3 crystal nhỏ xung quanh
      for (let k = 0; k < 3; k++) {
        const [ox, oz] = annulusPoint(center.x * WORLD_SCALE, center.z * WORLD_SCALE, 1.5, 3.5);
        const small = crystal.clone();
        shadow(small); nightify(small, 0.7, new THREE.Color(0.4, 1.0, 0.6));
        normalizeScale(small, 0.6 + Math.random() * 0.6);
        small.position.set(ox, 0, oz);
        groundSnap(small);
      ...
      // Glow light
      const glow = new THREE.PointLight(0x44ff88, 2.5, 10, 2);
      glow.position.set(center.x * WORLD_SCALE, 1.5, center.z * WORLD_SCALE);
      scene.add(glow); disposableLights.push(glow);
    }
  }).catch(e => console.error("❌ crystal:", e));

  // ── Crystal hồng ──────────────────────────────────────────────────────────
  propLoad(`${PROP_BASE}/crystal_hong.glb`).then(crystalHong => {
    const centers = [{ x: -8, z: -25 }, { x: 14, z: 42 }];
    for (const center of centers) {
      const main = crystalHong.clone();
      shadow(main); nightify(main, 0.7, new THREE.Color(1.0, 0.6, 0.8));
      normalizeScale(main, 1.6);
      main.position.set(center.x * WORLD_SCALE, 0, center.z * WORLD_SCALE);
      groundSnap(main);
      main.rotation.y = Math.random() * Math.PI * 2;
      scene.add(main);
      ...
      for (let k = 0; k < 2; k++) {
        const [ox, oz] = annulusPoint(center.x * WORLD_SCALE, center.z * WORLD_SCALE, 1.2, 2.8);
        const small = crystalHong.clone();
        shadow(small); nightify(small, 0.7, new THREE.Color(1.0, 0.6, 0.8));
        normalizeScale(small, 0.5 + Math.random() * 0.5);
        small.position.set(ox, 0, oz);
        groundSnap(small);
        small.rotation.y = Math.random() * Math.PI * 2;
        scene.add(small);
      ...
      const glow = new THREE.PointLight(0xff66aa, 2.0, 8, 2);
      glow.position.set(center.x * WORLD_SCALE, 1.2, center.z * WORLD_SCALE);
      scene.add(glow); disposableLights.push(glow);
    }
  }).catch(e => console.error("❌ crystal_hong:", e));
  ...
  propLoad(`${PROP_BASE}/stylized_wooden_wagon.glb`).then(wagon => {
    ...
    cloneAndPlace(scene, wagon, positions, CFG.MODEL_SCALE.bigStone * 1.2, 0,
      [0.4, new THREE.Color(0.75, 0.65, 0.5)]);
  ...
  propLoad(`${PROP_BASE}/stylized_medieval_house.glb`).then(house => {
    ...
    scene.add(house);
  }).catch(e => console.error("❌ house1:", e));
  ...
  propLoad(`${PROP_BASE}/stylized_medieval_house_2.glb`).then(house2 => {
    ...
    scene.add(house2);
  }).catch(e => console.error("❌ house2:", e));
  ...
  for (let i = 0; i < 150; i++) {
    ...
    if (!src) continue;
    const g = src.clone();
    normalizeScale(g, CFG.MODEL_SCALE.grass + Math.random() * 0.4);
    g.position.set(x, 0, z);
    groundSnap(g);
    g.rotation.y = Math.random() * Math.PI * 2;
