...
// ─── Zone offsets — khớp với PORTAL_DEFS trong HubConfig ─────────────────────
const ZONE = {
  HUB:            new THREE.Vector3(  0, 0,   0),
  LEFT_FOREST:    new THREE.Vector3(-40, 0,   0),
  RIGHT_PLATFORM: new THREE.Vector3( 40, 0,   0),
  BOSS_ARENA:     new THREE.Vector3(  0, 0, -80),
} as const;
...
export class WorldScene extends BaseScene {
  ...
  private _buildLeftForest(): void {
    ...
    this._addPlane(60, 60, 0x141f0e, O);
    [
      new THREE.Vector3(O.x - 15, 1.5, O.z - 20),
      new THREE.Vector3(O.x + 10, 2.0, O.z + 15),
      new THREE.Vector3(O.x - 10, 1.2, O.z + 10),
      new THREE.Vector3(O.x +  5, 1.8, O.z - 15),
    ].forEach((pos, i) => {
      const orb = new THREE.Mesh(
        new THREE.SphereGeometry(0.12, 8, 8),
        new THREE.MeshStandardMaterial({ color: 0x22ff88, emissive: new THREE.Color(0x22ff88), emissiveIntensity: 2.0 })
      );
    ...
    this._instanceProp("bush", O, 15, 25);
    this._placeProp("Big-stone",  new THREE.Vector3(O.x - 10, 0, O.z - 10), 2.0);
    this._placeProp("bo_ba_nam",  new THREE.Vector3(O.x + 12, 0, O.z +  5), 3.0);
  }

  private _buildRightPlatform(): void {
    const O = ZONE.RIGHT_PLATFORM;
    this._addPlane(60, 60, 0x1a140f, O, true);
    const platform = new THREE.Mesh(
      new THREE.CylinderGeometry(12, 14, 0.4, 16),
    ...
    [
      new THREE.Vector3(O.x - 10, 0.3, O.z - 10),
      new THREE.Vector3(O.x + 10, 0.3, O.z + 12),
    ].forEach((pos, i) => {
      const stone = new THREE.Mesh(
        new THREE.DodecahedronGeometry(0.4, 0),
        new THREE.MeshStandardMaterial({ color: 0x2a0055, emissive: new THREE.Color(0x6622ff), emissiveIntensity: 0.8 })
      );
    ...
    this._placeProp("crystal_hong",           new THREE.Vector3(O.x + 10, 0, O.z - 8 ), 1.5);
    this._placeProp("crystal_cluster",        new THREE.Vector3(O.x - 10, 0, O.z + 10), 2.0);
    this._placeProp("old_ropebridge_low_poly", new THREE.Vector3(O.x,      0, O.z - 20), 1.5);
  }

  private _buildBossArena(): void {
    const O = ZONE.BOSS_ARENA;
    this._addPlane(100, 100, 0x0d0808, O);
    ...
    const border = new THREE.Mesh(
      new THREE.RingGeometry(34, 36.5, 64),
      new THREE.MeshStandardMaterial({ color: 0x330000, emissive: new THREE.Color(0x550000), emissiveIntensity: 0.4, side: THREE.DoubleSide, depthWrite: false })
    );
    ...
    const fireBall = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xff4400, emissive: new THREE.Color(0xff2200), emissiveIntensity: 3.0, transparent: true, opacity: 0.85 })
    );
    fireBall.position.set(O.x, 1.5, O.z); this.scene.add(fireBall);

    this.centralFire = new THREE.PointLight(0xff3300, 5.0, 80);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const px = O.x + Math.cos(angle) * 32;
      const pz = O.z + Math.sin(angle) * 32;
      const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.6, 5, 8), pillarMat);
      shaft.position.set(px, 2.5, pz); shaft.castShadow = true; this.scene.add(shaft);
      const fb = new THREE.Mesh(
        new THREE.SphereGeometry(0.22, 8, 8),
        new THREE.MeshStandardMaterial({ color: 0xff6600, emissive: new THREE.Color(0xff6600), emissiveIntensity: 2.0 })
      );
      fb.position.set(px, 5.8, pz); this.scene.add(fb);
      if (i % 2 === 0) {
        const fl = new THREE.PointLight(0xff6600, 2.0, 15);
        fl.position.set(px, 6.0, pz);
        this.scene.add(fl);
    ...
    this._placePropMulti("pile_of_skulls", [
      new THREE.Vector3(O.x + 8,  0, O.z +  8),
      new THREE.Vector3(O.x - 8,  0, O.z + 12),
      new THREE.Vector3(O.x + 6,  0, O.z - 15),
    ], 0.3);
  }
  ...
  private _instanceProp(name: string, center: THREE.Vector3, count: number, spread: number): void {
    ...
    src.traverse((child) => {
      ...
      im.receiveShadow = true; im.frustumCulled = true;
      for (let i = 0; i < count; i++) {
  ...
  private _spawnEnemies(): void {
    ...
    // Left Forest
    this.enemyManager.spawn(
      [new THREE.Vector3(ZONE.LEFT_FOREST.x + 10, 0, ZONE.LEFT_FOREST.z + 10),
       new THREE.Vector3(ZONE.LEFT_FOREST.x - 8,  0, ZONE.LEFT_FOREST.z - 10)],
      { ...GOBLIN_CONFIG, chaseRange: 20 }
    );
    // Right Platform
    this.enemyManager.spawn(
      [new THREE.Vector3(ZONE.RIGHT_PLATFORM.x + 10, 0, ZONE.RIGHT_PLATFORM.z + 8),
       new THREE.Vector3(ZONE.RIGHT_PLATFORM.x - 12, 0, ZONE.RIGHT_PLATFORM.z + 10)],
      { ...GOBLIN_CONFIG, chaseRange: 18 }
    );
    // Boss — scale 2.5 thay vì 8.0
    this.enemyManager.spawn(
      [new THREE.Vector3(ZONE.BOSS_ARENA.x, 0, ZONE.BOSS_ARENA.z - 12)],
      { ...GOBLIN_CONFIG, scale: 2.5, maxHp: 500, chaseRange: 50, attackDamage: 25, patrolRadius: 0 }
    );
  }
