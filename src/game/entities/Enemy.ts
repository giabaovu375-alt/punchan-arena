...
// BUG FIX: lưu thêm rawHeight để tránh tính lại bbox trên object đã bị scale
// ─────────────────────────────────────────────────────────────────────────────
interface CachedModel {
  src:       THREE.Group;
  rawHeight: number; // bbox height tại scale=1
}
...
function loadModelCached(url: string): Promise<CachedModel> {
  ...
  const p = new Promise<CachedModel>((res, rej) => {
    const onLoaded = (group: THREE.Group) => {
      // Reset hoàn toàn transform — đảm bảo bbox đo ở scale=1
    ...
    if (url.endsWith(".glb") || url.endsWith(".gltf")) {
      ...
      new FBXLoader().load(url, (fbx) => {
        // FBXLoader auto-scale x0.01 (cm→m) được áp lên children bên trong,
        // nhưng root vẫn scale=1 sau khi ta reset.
        // → rawHeight sẽ phản ánh đúng kích thước FBX gốc (đơn vị cm nếu là cm-file)
...
export interface EnemyConfig {
  ...
  targetHeight?:   number;
  /** Scale multiplier nhân thêm SAU khi normalize height. Boss dùng 2.5, thường để 1. */
  scale?:          number;
  maxHp?:          number;
  moveSpeed?:      number;
  chaseRange?:     number;
...
export class Enemy {
  ...
  constructor(
    ...
    this.cfg = {
      ...
      scale:          config.scale          ?? 1.0,   // FIX: merge scale field
  ...
  private async loadAssets() {
    try {
      // FIX: lấy rawHeight từ cache thay vì đo lại trên object đã bị mutate
      const { src, rawHeight } = await loadModelCached(this.cfg.modelUrl);

      // clone() để không mutate src trong cache
      const model = src.clone(true);
      model.position.set(0, 0, 0);
      model.rotation.set(0, 0, 0);
      model.scale.set(1, 1, 1);

      // FIX: finalScale = normalize về targetHeight × scale multiplier (boss dùng 2.5)
      let finalScale = 1.0;
      if (rawHeight > 0.001) {
