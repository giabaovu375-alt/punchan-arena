...
export class GameEngine {
  ...
  private setupModel(model: THREE.Group, clips: AnimClipMap) {
    ...
    // Collect valid bone names để filter tracks
    ...
    for (const key of ANIM_KEYS) {
      ...
      // Deep clone từng track — THREE.AnimationClip.clone() là shallow clone
      ...
      const clip = new THREE.AnimationClip(src.name, src.duration, tracks, src.blendMode);
    ...
    this.mixer.addEventListener("finished", (e: any) => {
      const doneKey = (Object.entries(this.actions) as [AnimKey, THREE.AnimationAction][])
        .find(([, a]) => a === e.action)?.[0];
      if (doneKey && (COMBAT_ANIMS.has(doneKey) || doneKey === "jump")) {
        this.isAttacking = false;
        this.currentAction = null;
        this.currentKey = "idle";
        this.playAnim(this.isMovingNow() ? "walk" : "idle", 0);
      }
    });
