...
export class GameEngine {
  ...
  private setupModel(model: THREE.Group, clips: AnimClipMap) {
    ...
    for (const key of ANIM_KEYS) {
      ...
      // Tắt root motion — lọc .position track của bone gốc
      // Mixamo: "mixamorig:Hips" / "Hips" / "hips"
      // VRoid:  "J_Bip_C_Hips"
      // Generic: "Root" / "root"
  ...
  private playAnim(key: AnimKey, fade = 0.2) {
    ...
    if (!next) return;
    // Skip nếu đang chạy đúng key này rồi — tránh crossFadeTo mỗi frame → T-pose
  ...
  private update(dt: number) {
    ...
    // Ground check — dùng threshold 0.05 để tránh floating point flip
    // onGround = false chỉ khi thật sự đang ở trên không, không phải dao động
