...
// Ưu tiên load: idle trước (chặn render đẹp), rồi locomotion, rồi combat
...
// ─── Singletons + cache (sống xuyên suốt page) ───────────────────────────────
const _gltfLoader = /* @__PURE__ */ new GLTFLoader();
const _fbxLoader  = /* @__PURE__ */ new FBXLoader();

// Cache theo URL → tránh tải lại khi respawn / đổi nhân vật
const _gltfCache = new Map<string, Promise<THREE.Group>>();
const _clipCache = new Map<string, Promise<THREE.AnimationClip | null>>();

function loadGLTFOnce(url: string): Promise<THREE.Group> {
  ...
  // Clone để mỗi instance có scene riêng (giữ skeleton)
  return p.then((scene) => scene);
}

function loadClipOnce(url: string, key: AnimKey): Promise<THREE.AnimationClip | null> {
  let p = _clipCache.get(url);
  if (!p) {
    p = new Promise<THREE.AnimationClip | null>((resolve) => {
      _fbxLoader.load(
        url,
        (fbx) => {
          ...
          if (clip) {
            clip.name = key;
            // Strip prefix mixamorig: trên track names → retarget chuẩn
        ...
        undefined,
        (err) => {
          console.warn(`[GameEngine] Không load được animation: ${url}`, err);
          resolve(null);
        },
      );
    });
...
export class GameEngine {
  ...
  private async loadCharacterModel() {
    // ── Kick off ALL downloads in parallel ngay từ đầu (model + tất cả clips)
    // Model phải xong trước khi mixer được tạo; clips có thể attach sau.
    const modelPromise = loadGLTFOnce(this.character.modelUrl);
    const clipPromises = new Map<AnimKey, Promise<THREE.AnimationClip | null>>();
    for (const key of ANIM_PRIORITY) {
      clipPromises.set(key, loadClipOnce(`${ANIM_BASE}/${ANIM_CLIPS[key]}`, key));
    }
    ...
    try {
      ...
      // Mỗi instance dùng scene riêng để tránh chia sẻ transform
    ...
    model.traverse((child) => {
      ...
      if (m.isMesh) {
        m.castShadow = true;
        m.receiveShadow = true;
        // Frustum culling đã default true; tắt nếu là skinned để tránh pop
    ...
    // ── Attach idle TRƯỚC (priority) để có gì đó play ngay
    const idleClip = await clipPromises.get("idle")!;
    if (this.disposed) return;
    ...
    // ── Phần còn lại attach dần khi xong, theo thứ tự ưu tiên ────────────────
    for (const key of ANIM_PRIORITY) {
      if (key === "idle") continue;
      clipPromises.get(key)!.then((clip) => {
        if (this.disposed || !this.mixer || !clip) return;
