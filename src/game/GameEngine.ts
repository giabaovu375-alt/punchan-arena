...
const COMBAT_ANIMS: AnimKey[] = ["punch","kick","uppercut","dropKick","mmaKick","elbow","pain"];
...
export class GameEngine {
  ...
  // Placeholder bob (chỉ dùng khi chưa load xong model)
  ...
  constructor(container: HTMLElement, character: CharacterDef) {
    ...
    // ✅ Placeholder lên trước — không bao giờ undefined
    this.player = this.createPlayerMesh();
    this.scene.add(this.player);

    // Load real model async (swap khi xong)
    this.loadModelAsync();
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // WORLD (giữ nguyên từ bản gốc, chỉ thêm fireLight ref)
  // ═══════════════════════════════════════════════════════════════════════════
  private buildWorld() {
    ...
    const ground = new THREE.Mesh(
      groundGeo,
      new THREE.MeshStandardMaterial({ color: 0x6b8e4e, roughness: 0.95, metalness: 0, flatShading: true }),
    );
    ...
    const archParts = [
      { p: [-1.3, 0, 0] as [number,number,number], s: [0.8, 4, 0.8] as [number,number,number] },
      { p: [ 1.3, 0, 0] as [number,number,number], s: [0.8, 4, 0.8] as [number,number,number] },
      { p: [0, 3.2, 0]  as [number,number,number], s: [3.4, 0.8, 0.8] as [number,number,number] },
    ];
    const arch = new THREE.Group();
    for (const { p, s } of archParts) {
    ...
    this.fireLight.position.set(8, 1.5, 8);
    this.scene.add(this.fireLight);
  }

  // ═══════════════════════════════════════════════════════════════════════════
  // PLACEHOLDER — giống bản gốc, hiện ngay lập tức
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // LOAD MODEL THẬT — swap vào sau khi xong
  // ═══════════════════════════════════════════════════════════════════════════
  private async loadModelAsync() {
    ...
    try {
      const gltf = await new Promise<any>((res, rej) =>
        gltfLoader.load(this.character.modelUrl, res, undefined, rej),
      );
      model = gltf.scene as THREE.Group;
    } catch (err) {
      console.warn("[GameEngine] Không load được GLB, giữ placeholder:", err);
      return; // Giữ placeholder, game vẫn chạy bình thường
    }
    ...
    // Scale — Mixamo export thường cần 0.01
    model.scale.setScalar(0.01);

    // ✅ Swap: giữ position/rotation của placeholder
    model.position.copy(this.player.position);
    model.rotation.copy(this.player.rotation);
    this.scene.remove(this.player);
    this.player = model;
    this.scene.add(this.player);
    this.bodyParts = null;  // tắt placeholder bob
    this.modelLoaded = true;

    // AnimationMixer
    this.mixer = new THREE.AnimationMixer(this.player);

    // Load tất cả FBX animation song song
    const entries = Object.entries(ANIM_CLIPS) as [AnimKey, string][];
    await Promise.allSettled(
      entries.map(async ([key, file]) => {
        try {
          const fbx = await new Promise<THREE.Group>((res, rej) =>
            fbxLoader.load(`${ANIM_BASE}/${encodeURIComponent(file)}`, res as any, undefined, rej),
          );
          if (!fbx.animations.length || !this.mixer) return;
          const clip   = fbx.animations[0];
          clip.name    = key;
          const action = this.mixer.clipAction(clip);
          if (COMBAT_ANIMS.includes(key) || key === "jump") {
            action.setLoop(THREE.LoopOnce, 1);
            action.clampWhenFinished = true;
          }
          this.actions[key] = action;
        } catch {
          console.warn(`[GameEngine] Bỏ qua animation: ${file}`);
        }
      }),
    );

    // Finished listener — về idle sau combat/jump
    this.mixer.addEventListener("finished", () => {
      if (COMBAT_ANIMS.includes(this.currentKey) || this.currentKey === "jump") {
  ...
  private buildMobileUI() {
    ...
    // ── Nút action phải ───────────────────────────────────────────────────────
    const btnDefs = [
      { emoji: "👊", color: "rgba(255,110,40,0.8)",  bottom: "108px", right: "48px",  action: () => this.triggerAttack("punch")    },
      { emoji: "🦵", color: "rgba(240,50,70,0.8)",   bottom: "48px",  right: "112px", action: () => this.triggerAttack("kick")     },
      { emoji: "⬆️", color: "rgba(60,160,255,0.8)",  bottom: "48px",  right: "42px",  action: () => { if (this.onGround) { this.velocity.y = this.jumpSpeed; this.onGround = false; } } },
      { emoji: "💥", color: "rgba(170,60,255,0.8)",  bottom: "108px", right: "114px", action: () => this.triggerAttack("mmaKick")  },
      { emoji: "⚡", color: "rgba(255,200,30,0.8)",  bottom: "48px",  right: "178px", action: () => { this.input.sprint = !this.input.sprint; } },
    ];
    for (const def of btnDefs) {
      const btn = document.createElement("div");
      Object.assign(btn.style, {
        position: "absolute", bottom: def.bottom, right: def.right,
        width: "58px", height: "58px", borderRadius: "50%",
        background: def.color, border: "2px solid rgba(255,255,255,0.4)",
        backdropFilter: "blur(4px)",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: "22px", pointerEvents: "all", touchAction: "none",
    ...
    window.addEventListener("touchmove", (e) => {
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        // Camera swipe (nửa phải màn hình)
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // KEYBOARD + MOUSE (giữ nguyên bản gốc, thêm combat keys)
  // ═══════════════════════════════════════════════════════════════════════════
  private bindEvents() {
  ...
  private onWheel = (e: WheelEvent) => {
    ...
    this.targetDistance  = Math.max(2.5, Math.min(18, this.targetDistance));
  };
  private onResize = () => {
  ...
  private update(dt: number) {
    ...
    // Placeholder bob (khi chưa load model xong)
    this.animTime += dt;
    if (this.bodyParts) {
    ...
    // Animation state machine (chỉ khi model đã load)
