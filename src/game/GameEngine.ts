...
export class GameEngine {
  ...
  private playerFloor = 0;   // y mặt đất tại spawn (flat zone = 0)
  ...
  constructor(
    ...
    // Adaptive pixel ratio: mobile thường có DPR cao (3x) → rất tốn, cap ở 1.5 trên mobile
  ...
  private setupModel(model: THREE.Group, clips: AnimClipMap) {
    ...
    for (const key of ANIM_KEYS) {
      ...
      // FBX từ Mixamo export ở 30fps → duration thường ~1-3s, ổn
      // Nếu clip duration > 10s thì bị scale sai → normalize về max 3s
      if (clip.duration > 10) {
        const scale = clip.duration / 2;
        clip.tracks.forEach(t => {
          // scale lại thời gian các keyframe
          (t as any).times = (t as any).times.map((v: number) => v / scale);
        });
        clip.duration = clip.duration / scale;
      }

      // Tắt root motion: xoá track position của bone root/hips
      clip.tracks = clip.tracks.filter(t => {
        const boneName = t.name.split(".")[0].toLowerCase();
        return !(
          (boneName === "root" || boneName === "hips" || boneName === "j_bip_c_hips") &&
  ...
  private playAnim(key: AnimKey, fade = 0.2) {
    ...
    if (this.currentKey === key && this.currentAction && this.currentAction.weight > 0.9) return;
    const next = this.actions[key];
    if (!next) return;
    if (this.currentAction && this.currentAction !== next) {
      // crossFadeTo mượt hơn fadeOut+fadeIn riêng lẻ
  ...
  private buildWorld(isMobile = false) {
    ...
    sun.castShadow = true;
    // Mobile: shadow map 1024 thay vì 2048 → tiết kiệm ~4x GPU memory
    ...
    // Ground — flatten vùng trung tâm r<20 để player không bị lún
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      // r<20: phẳng hoàn toàn để player spawn/di chuyển ổn định
  ...
  private buildMobileUI() {
    ...
    // ── Lock landscape bằng meta + CSS ──────────────────────────────────────
    // Inject CSS lock landscape nếu chưa có
    if (!document.getElementById("__game-landscape-style")) {
      const s = document.createElement("style");
      s.id = "__game-landscape-style";
      s.textContent = `
        @media screen and (orientation: portrait) {
          #__game-rotate-overlay {
    ...
    // Overlay yêu cầu xoay màn hình khi portrait
    if (!document.getElementById("__game-rotate-overlay")) {
      ...
      Object.assign(ov.style, {
        ...
        gap:"16px", color:"#fff", fontFamily:"system-ui, sans-serif",
      });
      ov.innerHTML = `
        <div style="font-size:56px;animation:spin90 1.5s ease-in-out infinite alternate">📱</div>
        <div style="font-size:16px;font-weight:700;letter-spacing:0.1em">XOAY NGANG ĐỂ CHƠI</div>
        <div style="font-size:12px;opacity:0.5">Rotate device to landscape</div>
    ...
    // ── Joystick — bottom-left, safe area aware ──────────────────────────────
    ...
    // ── Action buttons — diamond layout bottom-right ─────────────────────────
    let sprintActive = false;
    const BTN_SIZE = "62px";
    const btns: { e:string; label:string; c:string; b:string; r:string; fn:()=>void; isToggle?:boolean }[] = [
      { e:"⬆️", label:"JUMP",    c:"#1a8cff", b:"148px", r:"108px", fn:()=>{ if(this.onGround){ this.velocity.y=this.jumpSpeed; this.onGround=false; } } },
      { e:"👊", label:"PUNCH",   c:"#ff6920", b:"82px",  r:"172px", fn:()=>this.triggerAttack("punch")   },
      { e:"🦵", label:"KICK",    c:"#e0143c", b:"16px",  r:"108px", fn:()=>this.triggerAttack("kick")    },
      { e:"💥", label:"SPECIAL", c:"#9b30ff", b:"82px",  r:"44px",  fn:()=>this.triggerAttack("mmaKick") },
      { e:"⚡", label:"SPRINT",  c:"#e6b800", b:"82px",  r:"250px", fn:()=>{}, isToggle:true },
    ];

    for (const def of btns) {
      const btn = document.createElement("div");
      Object.assign(btn.style, {
        position:"absolute", bottom:def.b,
        right:`max(${def.r}, calc(${def.r} + env(safe-area-inset-right, 0px)))`,
      ...
      icon.textContent = def.e;
      const lbl = document.createElement("span");
      lbl.textContent = def.label;
      Object.assign(lbl.style, {
        fontSize:"7px", color:"rgba(255,255,255,0.8)",
        fontFamily:"system-ui,sans-serif", fontWeight:"700", letterSpacing:"0.5px",
      });
      ...
      if (def.isToggle) {
        btn.addEventListener("touchstart", (e) => {
          ...
          btn.style.background = sprintActive ? "#e6b800ee" : "#e6b800cc";
          btn.style.boxShadow = sprintActive
            ? `0 0 20px rgba(230,184,0,0.75), 0 4px 18px rgba(0,0,0,0.45)`
            : `0 4px 18px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.18)`;
        }, { passive:false });
        btn.addEventListener("touchend", () => { btn.style.transform="scale(1)"; });
      } else {
        btn.addEventListener("touchstart", (e) => {
          e.preventDefault();
          btn.style.transform="scale(0.87)";
          btn.style.boxShadow="0 1px 6px rgba(0,0,0,0.4)";
          def.fn();
        }, { passive:false });
        btn.addEventListener("touchend", () => {
          btn.style.transform="scale(1)";
          btn.style.boxShadow="0 4px 18px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.18)";
        });
      }
      ui.appendChild(btn);
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  private start() {
    // Reset clock ngay trước khi bắt đầu loop để tránh spike dt lớn
    this.clock.start();
    const tick = () => {
      if(this.disposed) return;
      // Gọi getDelta() đúng 1 lần duy nhất mỗi frame
      const rawDt = this.clock.getDelta();
      const dt    = Math.min(rawDt, 0.05); // cap 50ms (20fps minimum)
      this.update(dt);
  ...
  // Cache vectors — tránh new Vector3() mỗi frame (giảm GC pressure)
  ...
  private update(dt: number) {
    ...
    if(this.player.position.y <= this.playerFloor){
      this.player.position.y = this.playerFloor; this.velocity.y=0; this.onGround=true;
    }
    ...
    // Anim state machine — update mixer TRƯỚC rồi mới quyết định anim tiếp theo
    if(this.mixer){
      this.mixer.update(dt);
      if(!this.isAttacking){
        if(!this.onGround)        this.playAnim("jump", 0.15);
        else if(moving && this.input.sprint) this.playAnim("run",  0.25);
    ...
    const tgt = this._tgt.copy(this.player.position).add(
  ...
  dispose(){
    ...
    // Cleanup landscape overlay
    document.getElementById("__game-rotate-overlay")?.remove();
    document.getElementById("__game-landscape-style")?.remove();
