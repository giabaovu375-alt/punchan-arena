...
export class GameEngine {
  ...
  private setupModel(model: THREE.Group, clips: AnimClipMap) {
    ...
    // Thu thập tên tất cả nodes/bones trong model
    // Dùng để filter track không có bone tương ứng → tránh T-pose
    ...
    for (const key of ANIM_KEYS) {
      ...
      // 1) Filter track không có bone trong model → tránh "No target node" → T-pose
      clip.tracks = clip.tracks.filter(t => {
        const boneName = t.name.split(".")[0];
        return nodeNames.has(boneName);
      });

      // 2) Tắt root motion — lọc .position của bone gốc
      clip.tracks = clip.tracks.filter(t => {
        if (!t.name.includes(".position")) return true;
        const raw = t.name.split(".")[0];
        const bn  = raw.toLowerCase().replace("mixamorig:", "").replace("mixamorig", "");
    ...
    // ── FIX T-POSE ──────────────────────────────────────────────────────────
    // Ép play 1 clip ngay để bind pose biến mất. Idle ưu tiên, không có thì
    // bất kỳ clip nào (walk/run/punch...) miễn là không phải bind pose.
    const startKey: AnimKey =
      this.actions.idle ? "idle" :
      this.actions.walk ? "walk" :
      this.actions.run  ? "run"  :
      (this.fallbackAnimKey ?? "idle");
    this.playAnim(startKey, 0);
    // Tick mixer 1 frame để evaluate pose ngay lập tức (tránh 1 frame T-pose)
  ...
  private playAnim(key: AnimKey, fade = 0.2) {
    ...
    if (!next) return;
    if (this.currentKey === key && this.currentAction === next) return;
  ...
  private update(dt: number) {
    ...
    // Ground check với threshold để tránh floating point flicker
    // → tránh playAnim("jump") loop vô tận mỗi frame → T-pose
    ...
    // Nếu nằm giữa 0.05 và 0.15 → giữ nguyên onGround trước đó (không flip)
