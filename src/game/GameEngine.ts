...
export class GameEngine {
  ...
  private setupModel(model: THREE.Group, clips: AnimClipMap) {
    ...
    // playerFloor phải bằng 0 vì player group y=0 là mặt đất
    // model.position.y = footOffset chỉ dịch visual, không ảnh hưởng physics
    this.playerFloor = 0;
  ...
  private playAnim(key: AnimKey, _fade = 0.2) {
    ...
    const isCombatOrJump = COMBAT_ANIMS.has(key) || key === "jump";
    if (isCombatOrJump || !next.isRunning()) {
      next.reset();
    }
    next.enabled = true;
  ...
  private update(dt: number) {
    ...
    if (this.player.position.y <= this.playerFloor + 0.02) {
      this.player.position.y = this.playerFloor;
      this.velocity.y = 0;
      this.onGround = true;
    } else if (this.player.position.y > this.playerFloor + 0.1) {
      this.onGround = false;
    }
