...
export class GameEngine {
  ...
  constructor(
    ...
    this.playerCtrl.setColliders([]); // intro map không cần collider phức tạp
  ...
  // SCENE SWITCH
  // ═══════════════════════════════════════════════════════════════════════════
  private switchToWorld() {
    // Xoá hết object intro khỏi scene
  ...
  private onMouseDown = (e: MouseEvent) => {
    ...
    if (e.button === 0 && this.sceneMode === "world") this.animCtrl.triggerAttack("punch");
  };
  ...
  private update(dt: number) {
    ...
    if (this.sceneMode === "intro" && this.introHandles) {
      ...
      // Trigger dialogue khi lại gần NPC
      if (
        !this.npcTriggered &&
        !locked &&
