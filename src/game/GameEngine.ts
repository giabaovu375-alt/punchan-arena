...
export class GameEngine {
  ...
  private playAnim(key: AnimKey, fade = 0.2) {
    ...
    // Chỉ skip nếu đúng key VÀ action chưa bị fade out (weight > 0.9)
  ...
  private buildMobileUI() {
    ...
    Object.assign(joyBase.style, {
      ...
      boxShadow:"0 0 0 1px rgba(255,255,255,0.08), inset 0 1px 0 rgba(255,255,255,0.15)",
    ...
    Object.assign(joyKnob.style, {
      ...
      boxShadow:"0 2px 16px rgba(0,0,0,0.4), 0 0 8px rgba(255,255,255,0.3)",
      position:"relative", transition:"transform 0.05s ease",
    });
    ...
    // ── Nút action ──────────────────────────────────────────────────────────
    // Layout: diamond pattern bên phải
    //         ⬆️ jump (top)
    //   👊 punch     💥 special
    //         🦵 kick (bottom)   ⚡ sprint (far right)
    let sprintActive = false;
    const btns: {
      e: string; label: string;
      c: string; b: string; r: string;
      fn: () => void;
      isToggle?: boolean;
      ref?: { el: HTMLElement | null };
    }[] = [
      { e:"⬆️", label:"JUMP",    c:"#1a8cff", b:"160px", r:"120px", fn:()=>{ if(this.onGround){ this.velocity.y=this.jumpSpeed; this.onGround=false; } } },
      { e:"👊", label:"PUNCH",   c:"#ff6920", b:"88px",  r:"180px", fn:()=>this.triggerAttack("punch")   },
      { e:"🦵", label:"KICK",    c:"#e0143c", b:"24px",  r:"120px", fn:()=>this.triggerAttack("kick")    },
      { e:"💥", label:"SPECIAL", c:"#9b30ff", b:"88px",  r:"56px",  fn:()=>this.triggerAttack("mmaKick") },
      { e:"⚡", label:"SPRINT",  c:"#e6b800", b:"88px",  r:"260px", fn:()=>{}, isToggle:true },
    ];

    for (const def of btns) {
      const btn = document.createElement("div");
      const sprintRef = { el: btn };
      ...
      Object.assign(btn.style, {
        ...
        pointerEvents:"all", touchAction:"none",
        boxShadow:"0 3px 16px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.2)",
      ...
      if (def.isToggle) {
        // Sprint toggle: visual state thay đổi khi bật/tắt
        btn.addEventListener("touchstart", (e) => {
          e.preventDefault();
          sprintActive = !sprintActive;
          this.input.sprint = sprintActive;
          btn.style.transform = "scale(0.92)";
          btn.style.background = sprintActive ? "#e6b800ff" : "#e6b800cc";
          btn.style.boxShadow = sprintActive
            ? "0 0 18px rgba(230,184,0,0.7), 0 3px 16px rgba(0,0,0,0.4)"
            : "0 3px 16px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.2)";
        }, { passive:false });
        ...
        btn.addEventListener("touchend", () => {
          ...
          btn.style.boxShadow = "0 3px 16px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.2)";
        });
      }
      ui.appendChild(btn);
    }
  ...
  private update(dt: number) {
    ...
    if(this.mixer){
      this.mixer.update(dt); // ← phải update trước để weight/time đúng
      if(!this.isAttacking){
