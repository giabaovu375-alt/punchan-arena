...
export function GameCanvas() {
  ...
  useEffect(() => {
    ...
    if (model) {
      const bones: string[] = [];
      model.traverse((c) => { if ((c as THREE.Bone).isBone) bones.push(c.name); });
      console.log("Model bone count:", bones.length);
      console.log("Model bones (first 10):", bones.slice(0, 10));
    }
    const walkClip = clipCache.get("walk");
    if (walkClip) {
      const trackNames = walkClip.tracks.slice(0, 8).map(t => t.name);
      console.log("Walk clip tracks (first 8):", trackNames);
    } else {
      console.log("Walk clip: NOT FOUND in cache");
    }
    console.log("Clips in cache:", [...clipCache.keys()]);
    // ──────────────────────────────────────────────────────────────────────────
