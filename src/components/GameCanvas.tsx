...
export function GameCanvas() {
  ...
  useEffect(() => {
    ...
    // Deep clone tracks trước khi truyền vào GameEngine
    // THREE.AnimationClip.clone() KHÔNG deep clone tracks
    // → GameEngine mutate tracks → clipCache bị corrupt → T-pose sau 2-3s
    const clips: AnimClipMap = {};
    for (const key of Object.keys(ANIM_MAP) as AnimKey[]) {
      const clip = clipCache.get(key);
      if (!clip) continue;
      clips[key] = new THREE.AnimationClip(
        clip.name,
        clip.duration,
