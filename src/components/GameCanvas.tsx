...
// ── Map AnimKey → đường dẫn FBX local ────────────────────────────────────────
...
// ── Cache toàn cục (persist qua re-render) ────────────────────────────────────
const modelCache = new Map<string, THREE.Group>();       // url → Group
const clipCache  = new Map<string, THREE.AnimationClip>(); // path → clip
...
export function GameCanvas() {
  ...
  // Mount engine với assets đã có sẵn trong cache
  useEffect(() => {
    if (stage !== "playing" || !ref.current || !selectedId) return;

    const char  = CHARACTERS.find((c) => c.id === selectedId)!;

    // Clone model để mỗi lần play là instance riêng
    const cachedModel = modelCache.get(char.modelUrl);
    const model = cachedModel ? (cachedModel.clone() as THREE.Group) : null;
  ...
  return (
    <div style={{
      ...
      <div style={{ position:"relative", zIndex:2, textAlign:"center", marginBottom:56 }}>
        <div style={{ fontSize:11, letterSpacing:"0.4em", color:"rgba(255,255,255,0.25)", textTransform:"uppercase", marginBottom:12 }}>
          Đang khởi động
        </div>
        <div style={{
          fontSize:"clamp(32px,6vw,56px)", fontWeight:900,
          letterSpacing:"0.08em", color:"#fff",
          textShadow:"0 0 60px rgba(100,140,255,0.4)", textTransform:"uppercase",
        }}>
          PLAY FRAME FORGE
        </div>
        <div style={{ marginTop:8, fontSize:12, color:"rgba(255,255,255,0.2)", letterSpacing:"0.2em", textTransform:"uppercase" }}>
          3D Action RPG
        </div>
      </div>
      <div style={{ position:"relative", zIndex:2, width:"min(400px,80vw)" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontSize:11, color:"rgba(255,255,255,0.3)", letterSpacing:"0.06em", maxWidth:"70%", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
            {label}
          </div>
          <div style={{ fontSize:13, fontWeight:700, color:"#6688ff", fontVariantNumeric:"tabular-nums" }}>
            {pct}%
          </div>
        </div>
      </div>
      <div style={{ position:"relative", zIndex:2, marginTop:48, display:"flex", gap:6 }}>
        {[0,1,2].map(i=>(
          <div key={i} style={{
            width:4, height:4, borderRadius:"50%", background:"#4466ff",
  ...
  return (
    <div className="relative flex h-screen w-screen items-center justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mt-4 font-serif text-6xl font-bold tracking-tight text-white" style={{ textShadow:`0 0 30px ${accent}` }}>
          {name}
        </div>
